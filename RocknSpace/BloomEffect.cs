...
namespace RocknSpace
{
    public class BloomEffect : ShaderEffect
    {
        ...
        static BloomEffect()
        {
            pixelShader.UriSource = MakePackUri("ShaderSource/Bloom.ps");
           
        }
        
        private static Uri MakePackUri(string relativeFile)
        {
            Assembly a = typeof(BloomEffect).Assembly;
            string name = a.ToString().Split(',')[0];

            StringBuilder uriString = new StringBuilder(); ;
            uriString.Append("/" + name + ";component/" + relativeFile);
